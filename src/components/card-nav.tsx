...
const CardNav: React.FC<CardNavProps> = ({
  ...
  useLayoutEffect(() => {
    // Reset cardsRef array to match items length
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between px-4 z-[2]">
          <Link href="/" className="logo-container flex items-center">
            <Image src={logo} alt={logoAlt} width={120} height={40} className="logo h-[40px] w-auto" />
          </Link>

          <div
            className={`hamburger-menu ${isHamburgerOpen ? 'open' : ''} group flex flex-col items-center justify-center cursor-pointer gap-[6px] p-2`}
            onClick={toggleMenu}
            role="button"
                ...
                {item.links?.map((lnk, i) => {
                  ...
                  if (lnk.onClick) {
                    return (
                      <button
                        key={`${lnk.label}-${i}`}
                        className="nav-card-link inline-flex items-center gap-[6px] no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px] bg-transparent border-none text-inherit p-0 text-left"
                        onClick={lnk.onClick}
                        aria-label={lnk.ariaLabel}
                      >
                        <IconComponent className="nav-card-link-icon shrink-0 w-4 h-4" aria-hidden="true" />
                        {lnk.label}
                      </button>
                    );
                  }
